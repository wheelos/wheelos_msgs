...
HEADER_PY_GRPC = """load("//bazel:python_rules.bzl", "py_grpc_library")
"""
...
TEMPLATE_PACKAGE_AGGREGATES = """
cc_library(
    name = "{package_name}_cc_proto",
    deps = [
{cc_deps}    ],
)

py_library(
name = "{package_name}_python_init",
srcs = ["__init__.py"],
)
...
def main(build_file_path):
    ...
    files_all = sorted([f for f in os.listdir(workdir) if
    ...
    for protofile in files_all:
        proto_deptext = generate_dependency_text(workdir, protofile)
        rules = generate_rule_for_protofile(workdir, protofile,
                                            proto_deptext)
        fout.write(rules)

    package_name = os.path.basename(workdir)
    cc_deps = "".join(
        TAB * 2 + '":{}_cc_proto",\n'.format(os.path.splitext(protofile)[0])
        for protofile in files_all)
    py_deps = "".join(
        TAB * 2 + '":{}_py_pb2",\n'.format(os.path.splitext(protofile)[0])
        for protofile in files_all)
    fout.write(TEMPLATE_PACKAGE_AGGREGATES.format(
...
def generate_rule_for_protofile(workdir, protofile, proto_deps):
...
def _google_proto_target(import_path):
    base = os.path.basename(import_path)
    (name, ext) = os.path.splitext(base)
    if ext != ".proto":
        return None
    return "@com_google_protobuf//:{}_proto".format(name)
...
def generate_dependency_text(workdir, protofile):
    ...
    for dep in dependencies:
        if dep.startswith("wheelos_msgs/"):
            dep = dep[len("wheelos_msgs/"):]
            depdir = "wheelos_msgs/" + os.path.dirname(dep)
        ...
        elif dep.startswith("google/protobuf/"):
            target = _google_proto_target(dep)
            if target:
                proto_notes.append(target)
        else:
            print("Warning: unsupported proto import {}, skipped.".format(dep))

    if len(proto_notes) == 0:
        return ""
